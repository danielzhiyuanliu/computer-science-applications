...
public class MediaLibRunner
{
  ...
  {
    MediaLib.owner = "Daniel";
    System.out.println(MediaLib.getOwner() + "'s Library");    
    MediaLib newLib = new MediaLib();

    Book newBook = new Book("Lord of the Rings", "Tolkien");
    newLib.addBook(newBook);
    // Book secBook = new Book("To Kill a Mockingbird", "Lee");
    // newLib.addBook(secBook);
    // Book thirdBook = new Book("1984", "Orwell"); 
    // newLib.addBook(thirdBook);

    MediaLib secLib = new MediaLib();

    Movie newMovie = new Movie("Dune", 2.5);
    secLib.addMovie(newMovie);

    Song newSong = new Song("Fly Me To The Moon", "Frank Sinatra"); 
    newLib.addSong(newSong);

    System.out.println(newLib);
    System.out.println(secLib);

    System.out.println("Total number of entries: " + MediaLib.getNumEntries());
    
    // System.out.println(MediaLib.changeOwner());
